...
import { Mail, Phone, MapPin, Wheat, ArrowUp } from "lucide-react";
...
const Footer = () => {
  ...
  return (
    <footer className="bg-spice-800 text-white pt-16 pb-8 relative overflow-hidden">
      {/* 3D Decorative Elements */}
      <div className="absolute -top-20 -right-20 w-40 h-40 rounded-full bg-spice-700 opacity-20 blur-3xl"></div>
      <div className="absolute -bottom-20 -left-20 w-40 h-40 rounded-full bg-leaf-600 opacity-10 blur-3xl"></div>
      ...
      <motion.div
        ...
        className="spice-container relative z-10"
      >
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
          <motion.div variants={itemVariants}>
            ...
            <p className="text-spice-200 mb-4">
              Premium quality spices sourced directly from sustainable farms, preserving flavor and aroma in every grain.
            </p>
            <div className="flex space-x-4">
              <motion.a 
                href="#" 
                className="text-white hover:text-spice-200 transition-colors"
          ...
          <motion.div variants={itemVariants}>
            ...
            <ul className="space-y-2">
              <motion.li whileHover={{ x: 5 }} transition={{ type: "spring", stiffness: 400, damping: 10 }}>
                <a href="#home" className="text-spice-200 hover:text-white transition-colors flex items-center gap-2">
                  <span className="text-xs opacity-60">→</span> Home
                </a>
              </motion.li>
              <motion.li whileHover={{ x: 5 }} transition={{ type: "spring", stiffness: 400, damping: 10 }}>
          ...
          <motion.div variants={itemVariants}>
            ...
            <ul className="space-y-2">
              <motion.li whileHover={{ x: 5 }} transition={{ type: "spring", stiffness: 400, damping: 10 }}>
                <a href="#products" className="text-spice-200 hover:text-white transition-colors flex items-center gap-2">
                  <span className="text-xs opacity-60">→</span> Cardamom
                </a>
              </motion.li>
              <motion.li whileHover={{ x: 5 }} transition={{ type: "spring", stiffness: 400, damping: 10 }}>
                <a href="#products" className="text-spice-200 hover:text-white transition-colors flex items-center gap-2">
                  <span className="text-xs opacity-60">→</span> Cinnamon
                </a>
              </motion.li>
              <motion.li whileHover={{ x: 5 }} transition={{ type: "spring", stiffness: 400, damping: 10 }}>
                <a href="#products" className="text-spice-200 hover:text-white transition-colors flex items-center gap-2">
                  <span className="text-xs opacity-60">→</span> Cumin Seeds
                </a>
              </motion.li>
              <motion.li whileHover={{ x: 5 }} transition={{ type: "spring", stiffness: 400, damping: 10 }}>
                <a href="#products" className="text-spice-200 hover:text-white transition-colors flex items-center gap-2">
        ...
        <div className="border-t border-spice-700 pt-6 mt-6 text-center relative">
          <motion.button
            onClick={handleScrollToTop}
            className="absolute -top-6 left-1/2 transform -translate-x-1/2 bg-spice-700 hover:bg-spice-600 w-12 h-12 rounded-full flex items-center justify-center shadow-lg transition-colors"
            whileHover={{ y: -3 }}
            whileTap={{ y: 1 }}
          >
